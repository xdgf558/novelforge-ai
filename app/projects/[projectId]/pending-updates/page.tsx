...
export default async function PendingUpdatesPage({
  ...
  const statusFilter = pendingUpdateStatusFilter(resolvedSearchParams?.status);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-700">
            AI
            只能把章节中提取到的变化放进待审核列表。只有作者点击批准后，更新才会写入正式设定、角色档案、世界规则、伏笔或时间线。
          </p>
        </div>
      </div>
      ...
      {automaticRecoveryCandidateCount > 0 ? (
        ...
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <form
              action={approveAutomaticForeshadowRecoveryBatch.bind(
                null,
                project.id,
              )}
            >
            ...
            {totalPages > 1 ? (
              <div className="nf-review-pagination">
                {page <= 1 ? (
                  ...
                  <Link
                    href={pendingUpdateHref(project.id, statusFilter, page - 1)}
                  >
                    上一页
                  </Link>
                )}
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
                          ...
                          {automaticRecovery ? (
                            <span className="rounded-md bg-signal-500/10 px-2.5 py-1 text-signal-700">
                              自动识别 ·{" "}
                              {automaticRecovery.confidence === "high"
                                ? "高置信"
                                : "需复核"}
                            </span>
                          ) : null}
                        ...
                        <dl className="mt-3 grid gap-2 text-sm text-ink-700 md:grid-cols-2">
                          <Row
                            label="影响对象"
                            value={update.targetName || "未指定"}
                          />
                          <Row
                            label="字段"
                            value={update.fieldName || "未指定"}
                          />
                          <Row
                            label="来源章节"
                            value={
                          ...
                          <Row
                            label="创建时间"
                            value={formatDate(update.createdAt)}
                          />
                          {update.appliedAt ? (
                            <Row
                              label="处理时间"
                              value={formatDate(update.appliedAt)}
                            />
                          ) : null}
                        </dl>
                      </div>
                    ...
                    <div className="mt-4 grid gap-4 lg:grid-cols-2">
                      <TextBlock
                        label="建议内容"
                        value={update.proposedContent}
                      />
                      <div className="space-y-4">
                        <TextBlock
                          label="提取原因"
                          value={update.reason || "未提供"}
                        />
                        <TextBlock
                          label="原文证据"
                          value={update.evidence || "未提供"}
                        />
                      </div>
                    </div>
                    ...
                    {update.aiTask ? (
                      <p className="mt-4 rounded-md bg-paper-50 px-3 py-2 text-xs leading-5 text-ink-700">
                        AI 任务：{update.aiTask.model} /{" "}
                        {update.aiTask.inputContextSummary}
                      </p>
                    ) : null}
                    ...
                    {isPending ? (
                      <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_260px]">
                        <form
                          action={approvePendingUpdate.bind(
                            null,
                            project.id,
                            update.id,
                          )}
                        ...
                        <form
                          action={rejectPendingUpdate.bind(
                            null,
                            project.id,
                            update.id,
                          )}
...
function ReviewReturnFields({
  ...
  return (
    <>
      ...
      <input name="returnUpdateId" type="hidden" value={updateId} />
    </>
  );
}
