...
- Packaged the approved next-story-unit outline continuation fixes from PR #98,
  merged into `main` at `8b7b143`, and bumped the source app/package version
  from `0.1.115` to `0.1.116`.
- Updated the in-app release notes for single-unit continuation, per-volume unit
  numbering, audited start-chapter copy matching, and explicit copy failures.
- Built the arm64 macOS application with hardened runtime and Developer ID
  Application signing.
- Built `release/desktop/NovelForge-AI-0.1.116-mac-arm64.pkg` with a payload at
...
- Removed the previous `0.1.115` installer, Electron build output, builder
  metadata, and isolated validation files so `release/desktop/` contains only
  the `0.1.116` PKG.

Verification:

- `npm test` passed: 122 files and 735 tests.
...
- Deep strict code-sign verification passed for the built app, an ordinary
  staging copy, and the app expanded from the final PKG.
- `pkgutil --check-signature` verified the final PKG's Developer ID Installer
  ...
  `com.novelforge.ai`, version `0.1.116`, and payload path
...
- Gatekeeper identifies the PKG as `Unnotarized Developer ID`, as expected for
  ...
  stapled.
- Final PKG size: 413,592,371 bytes.
- Final PKG SHA-256:
  `b75631b2738f68a15e51e56494a5528db84dc26a0aab08058b7636c7781d6673`.
...
- Packaged the approved continuity-report layout fix from PR #94, merged into
  `main` at `fa0af55`, and bumped the source app/package version from
  `0.1.112` to `0.1.113`.
- Updated the in-app release title and notes for the responsive continuity
  report detail layout: long AI findings now preserve usable detail width when
  the AI run console is open without changing report actions or repair flows.
- Built the arm64 macOS application with hardened runtime and Developer ID
  Application signing.
- Built `release/desktop/NovelForge-AI-0.1.113-mac-arm64.pkg` with a payload at
  `/Applications/NovelForge AI.app` and signed it with Developer ID Installer.
- Did not upload this rebuild to Apple notarization because the request only
  asked for a new installer package.
- Removed the previous `0.1.112` installer and all Electron build intermediates
  so `release/desktop/` contains only the `0.1.113` PKG.
...
- `npm run typecheck`, `npm run desktop:smoke`, `npm run mvp:acceptance`,
  `npm run work-types:acceptance`, and `npx prisma validate` passed.
- The production build inside `npm run desktop:dist:mac` passed.
- Deep strict code-sign verification passed for the packaged app and the app
  expanded from the final PKG.
- Host `pkgutil --check-signature` verified the final PKG's Developer ID
  Installer certificate chain and trusted timestamp. The final PKG reports
  identifier `com.novelforge.ai`, version `0.1.113`, and payload path
  `/Applications/NovelForge AI.app`.
- Packaged-root `npm run work-types:acceptance` passed against the final PKG's
  `app.asar.unpacked` resources. An isolated launch of that payload returned
  HTTP 200, applied all 26 bundled migrations, and returned `ok` from SQLite
...
- Gatekeeper identifies the PKG as `Unnotarized Developer ID`, as expected for
  a build that was not uploaded to Apple; this installer is not notarized or
  stapled.
- Final PKG size: 414,869,311 bytes.
- Final PKG SHA-256:
  `b14a48dd5a67868808eb244fdd851b82290777dabec168bde0470f7fbc0dd80f`.
...
- Merged the approved review-first short-story writing-unit planning feature
  into `main` at merge commit `065fd74`.
- Bumped the source app/package version from `0.1.97` to `0.1.98`.
- Rebuilt the arm64 macOS application with one-click editable unit-plan drafts,
  automatic task refresh, and atomic task adoption only after the author
  creates the formal writing unit.
...
- Created the formal installer at
  `release/desktop/NovelForge-AI-0.1.98-mac-arm64.pkg`, targeting
  `/Applications`.
...
- The app expanded from the final PKG passed system-level
  `codesign --verify --deep --strict`, retained all 25 migrations, and contains
  the unit-plan server action and structured context builder.
...
- No real overwrite install into `/Applications` was performed.
- Final size: `367,480,856` bytes. SHA-256:
  `016de299db5675021104a09f47467616f32eb2fa5a61d9fa883695e3ed858a55`.
...
- Merged the approved compact short-story series memory display into `main` at
  merge commit `1ded3eb`.
- Bumped the source app/package version from `0.1.96` to `0.1.97`.
- Rebuilt the arm64 macOS application with the compact series premise and
  shared-memory panels, including bounded scrolling for expanded long text.
- Signed the Electron app with `Developer ID Application: HAO YE
  (Y35K7AQ974)` and the installer with `Developer ID Installer: HAO YE
  (Y35K7AQ974)` plus a trusted timestamp.
- Created the formal installer at
  `release/desktop/NovelForge-AI-0.1.97-mac-arm64.pkg`, targeting
  `/Applications`.
- Kept the personal local build unnotarized because Apple notarization requires
  an external artifact upload and no explicit upload authorization was given.
...
- The app expanded from the final PKG passed system-level
  `codesign --verify --deep --strict`, retained all 25 migrations, and contains
  the compact series memory panels and bounded expand/collapse regions.
- Packaged `npm run work-types:acceptance` passed. ASAR inspection confirmed
  desktop startup still uses `runDesktopMigrations` and contains no Prisma CLI
  `migrate deploy` startup path.
- No real overwrite install into `/Applications` was performed.
- Final size: `362,865,266` bytes. SHA-256:
  `0ce2652fb4b710d10f0d3c4f85590b98bcd86c8e1c321e3f47349b2f4385b435`.
...
- Bumped the source app/package version from `0.1.91` to `0.1.92` after the
  reader-feedback retirement was reviewed and merged through PR #69.
- Rebuilt the arm64 macOS app payload without the chapter reader-feedback UI,
  Station Cat feedback fetch path, or feedback-driven beat/draft context. The
  legacy feedback schema remains bundled for non-destructive database upgrades.
- Copied the app into an isolated PKG staging root and re-signed the complete
  Electron bundle ad hoc with hardened runtime plus the required JIT, unsigned
  executable memory, and library-validation entitlements.
- Built the formal personal-use installer at
  `release/desktop/NovelForge-AI-0.1.92-mac-arm64.pkg`, targeting
  `/Applications`.
- Removed the `0.1.91` installer and Electron build intermediates so the
  delivery directory contains only the new verified PKG.

Verification:

- Full `npm run test` passed: 103 files and 565 tests.
- `npm run typecheck`, `npm run desktop:smoke`, `npm run mvp:acceptance`, and
  source-tree `npm run work-types:acceptance` passed.
- `npx prisma validate` passed, and `prisma migrate diff` reported no difference
  between all 23 migrations and `prisma/schema.prisma`.
- The production build inside `npm run desktop:pack:mac` passed. The original
  app payload passed Developer ID deep/strict signature verification and
  reported Bundle and packaged source version `0.1.92`.
- The re-signed staging payload and a second ordinary copy both passed
  deep/strict signature verification. The required hardened-runtime
  entitlements remained present.
- The final PKG metadata uses identifier `com.novelforge.ai`, version `0.1.92`,
  and install location `/Applications`.
- The app expanded from the final PKG reports Bundle and packaged source version
  `0.1.92`, passes deep/strict signature verification, retains all required
  entitlements, and contains all 23 bundled migrations.
- Packaged runtime inspection confirmed `runDesktopMigrations`, bundled
  migration SQL loading, and no Prisma CLI `migrate deploy` startup path.
- `npm run work-types:acceptance` passed again using the expanded PKG's
...
- The PKG itself is unsigned because no Developer ID Installer identity is
  ...
  overwrite install into `/Applications` was performed.
- Final SHA-256:
  `5d405048826fff38adb41c40ee1f44267a8050c74c267fff00edb7dec1a41568`.
