...
- Current source app version is `0.1.111`; the latest personal-use macOS
  installer is
  `release/desktop/NovelForge-AI-0.1.111-mac-arm64.pkg`. This release has a
  ...
  authorize a new Apple upload for this rebuild, so `0.1.111` is not notarized
  ...
  artifact as notarized or Gatekeeper-trusted. The previous notarized `0.1.110`
  artifact was removed at the author's request. Future handoff should leave
