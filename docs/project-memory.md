...
- Current source app version is `0.1.108`; the latest personal-use macOS installer is `release/desktop/NovelForge-AI-0.1.108-mac-arm64.pkg`. This specific release is an explicitly approved ad-hoc-signed app inside an unsigned, unnotarized PKG because no valid code-signing identities were available during the release; it is not a Developer ID distribution artifact. Future handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
