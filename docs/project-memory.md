...
- Long-form project completion is an explicit author action, never an AI-side
...
- For known default endpoints, task-route normalization is bidirectional:
  switching a saved Luna/OpenAI route back to a Kimi/Moonshot model restores
  the Moonshot default before a new key can be persisted or used. Manually
  entered custom endpoints remain author-controlled.
...
- The default structural/editorial route is fixed to `gpt-5.6-terra` at `https://api.openai.com/v1` with `reasoning: { effort: "xhigh" }` (shown as "极高" in the UI). It handles project settings, planning, outlines, beats, summaries, memory extraction, continuity checks, and other tasks that do not have a dedicated writing route. `OPENAI_MODEL` and `OPENAI_BASE_URL` are legacy inputs for migration detection and are not configurable switches; environment examples expose only `OPENAI_API_KEY`. Saved DeepSeek model ids, official DeepSeek endpoints, and other non-official default connections are ignored at runtime: the app selects Terra/OpenAI and omits the effective API key so a provider credential can never be sent to OpenAI. The old value remains only in the author's local file until Terra settings are saved, at which point it is removed. The settings page shows retired-provider guidance only when that migration marker is present; fresh installs receive a neutral OpenAI API Key prompt. Missing-key failures and the affected outline/chapter panels direct the author to `Settings -> Terra Access`. Chapter drafts may still use a dedicated route such as Kimi K2.6 or `gpt-5.6-luna`; chapter polish and short-story whole review may use the polish route with Kimi K3. Luna uses the same `xhigh` reasoning rule as Terra. Dedicated writing routes use `normalizeTaskRouteModel*`, never the default-route normalizer. A blank polish model suggests `kimi-k3`, while explicitly saved K2.6 routes remain unchanged. Authors may explicitly let polish reuse the draft route's API Key and Base URL; sharing never activates silently, and a dedicated polish key takes precedence. K3 Chat Completions requests keep top-level `reasoning_effort: "max"`. When a writing route has no effective saved API key, the entire connection falls back to Terra: model, Base URL, and credential all come from the default route, so a Kimi model is never sent to OpenAI. Routed tasks store a non-secret execution snapshot with route source, task type, model, and base URL in `ai_tasks.inputJson`, while API keys remain only in local config. Long-form writing and deep editorial tasks (`chapter_draft_generation`, `chapter_polish_generation`, and `short_story_whole_review`) use a 10-minute inactivity timeout. Chat Completions routes stream SSE, reset that timeout with each received chunk, and make output adoptable only after `data: [DONE]`; direct OpenAI Responses routes use the same request timeout without the Chat Completions stream. Planning, extraction, and continuity tasks stay non-streaming. Long planning or memory-audit tasks (`chapter_beat_generation`, `outline_generation`, `ending_planning_generation`, `pending_update_extraction`, and `foreshadow_recovery_audit`) use a 5 minute model request timeout because planning and JSON extraction responses can exceed 120 seconds even with compact inputs; shorter structural extraction/check tasks keep the default 120 second timeout. Timeout-aware direct `Agent` and proxy `ProxyAgent` dispatchers must set headers/body timeouts to the caller timeout plus a 30-second transport grace period, so Undici's roughly 300-second defaults cannot preempt longer AI tasks.
- Task-route credentials are connection-scoped. A saved route key is retained only when its connection identity remains compatible: recognized Moonshot/OpenAI model families require the same normalized Base URL, while an unrecognized custom model must retain both the exact model id and Base URL. Draft/polish reuse is even narrower: it requires the same recognized provider family and identical normalized Base URL, so Kimi routes may share a Kimi connection but a Luna or `gpt-4.1` draft can never share with K3; custom models require independent keys. Switching connection identity with a blank key explicitly clears the prior local route key, including any matching environment fallback. A queued task may use its non-secret model/base-URL snapshot only when the current route retains that same connection identity; otherwise it fails without sending a key to the snapshot endpoint.
...
- Phase 16: AI connection settings page for a local API Key and server-side model access without exposing API keys to the frontend. The original custom default-provider control was later narrowed to the fixed Terra/OpenAI structural route; Kimi/OpenAI writing routes remain separately configurable.
...
- Current source app version is `0.1.116`; the latest personal-use macOS
  installer is
  `release/desktop/NovelForge-AI-0.1.116-mac-arm64.pkg`. This release has a
  ...
  explicitly authorize a new Apple upload for this rebuild, so `0.1.116` is
  ...
  previous `0.1.115` artifact was removed at the author's request. Future
...
- Project deletion should remain archive-first. The default project list shows active projects; the archive directory contains both author-completed serials and manually archived projects, each of which can be restored from the edit page. Hard delete must keep a visible backup acknowledgement and typed confirmation.
...
- OpenAI's official base URL uses the Responses API, while dedicated custom OpenAI-compatible writing routes use `/chat/completions`. AI task failures should be recorded in `ai_tasks` and redirect back to the relevant page instead of surfacing a full Next.js application error page.
- Chapter detail pages auto-refresh while AI tasks are pending/running. Streamed long-form tasks update `AiTask.updatedAt` with throttled progress heartbeats, and stale chapter or short-story whole-review tasks are marked failed only after 15 minutes without activity so healthy long generations do not permanently lock buttons or get terminated mid-response. Packaged desktop startup should immediately mark every remaining active AI task as failed after migrations, because model requests live only in the previous Next.js process and cannot resume after an app restart. This cleanup is best effort and must never block application startup; the existing inactivity cleanup remains the fallback. Desktop and Next.js task locking share `lib/ai/active-task-statuses.json` as the runtime source of active statuses.
