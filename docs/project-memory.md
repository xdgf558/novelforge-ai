...
- Current formal personal-use macOS installer version is `0.1.1`; handoff should leave only `release/desktop/NovelForge-AI-0.1.1-mac-arm64.dmg` in the delivery folder unless the user explicitly asks for ZIP/update metadata.
- Before reporting a desktop package as ready, verify the mounted DMG app still uses `runDesktopMigrations` from `desktop/runtime.cjs`, does not contain Prisma CLI `migrate deploy` startup code, and can launch from a read-only DMG with a temporary user data directory.
