...
- Prompt template management is basic by design: authors can view/copy, duplicate a version, enable/disable, and reset to defaults. Do not add arbitrary browser-side API key access or hidden prompt mutation. AI calls should use the highest-version active template for a key, but if the code ships a newer default template version than the currently active project template, `ensureDefaultPromptTemplate` may auto-upsert and use that newer default. User-created higher-version templates still remain preferred.
