...
- Phase 25: audiobook export MVP, including local TTS settings, PPQ TTS provider support, ElevenLabs/DeepGram model options through PPQ, voice list fetching, voice preview playback, chapter-level segmented audio export, `audio_exports` / `audio_export_segments`, failed-segment retry, local export folder opening, and controlled `/audio-assets` playback.
...
- Current source app version is `0.1.44`; the latest formal personal-use macOS installer is `release/desktop/NovelForge-AI-0.1.44-mac-arm64.pkg`. Handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
...
- TTS / audiobook export config is now editable at `/ai-settings`; the same local `.env` stores `TTS_PROVIDER_ID`, `TTS_API_KEY`, `TTS_API_BASE_URL`, `TTS_MODEL`, `TTS_VOICE_ID`, `TTS_VOICE_NAME`, `TTS_LANGUAGE_CODE`, `TTS_OUTPUT_FORMAT`, and `TTS_STYLE_PROMPT`. The first supported provider is PPQ TTS with default base URL `https://api.ppq.ai/v1`; Google and Bailian remain future adapters, not active MVP providers.
- The TTS voice picker should remain explicit: refreshed voice lists can auto-select the first voice only for immediate preview convenience, but the author can click "保存当前音色" to persist the selected `TTS_VOICE_ID` / `TTS_VOICE_NAME` into the local `.env`. Preview/export actions should fail with a clear missing-voice message when no voice id is submitted. PPQ voice languages such as `multi`, `multilingual`, `auto`, `any`, or `all` are generic labels, not valid concrete `language_code` values; do not let them override the author's language field, and normalize legacy saved values back to `zh`.
- PPQ currently returns no DeepGram Aura 2 voices for `language=zh`; Chinese audiobook narration should default to ElevenLabs Multilingual v2 unless PPQ later adds Chinese DeepGram voices. When the user selects a DeepGram voice from the refreshed list, persist that voice's own language code (for example `ja` or `en`) along with the voice id so preview/export do not reuse stale `zh` language settings.
- TTS preview uses the same PPQ `/audio/speech` path as chapter export but is allowed local-network recovery retries: if the preview call fails with a transient proxy/network error such as `fetch failed`, reset the cached proxy dispatcher and retry up to 3 total attempts. If it still fails, surface the sanitized provider/runtime error in `/ai-settings` rather than hiding it behind a generic preview failure.
- Network proxy config is now editable at `/ai-settings` inside the AI connection section; the same local `.env` stores `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY`. Server-side OpenAI-compatible model calls, image generation, PPQ TTS, and Station Cat publishing use `createServerFetch`, which reads the local proxy config so GUI-launched desktop builds can access services that otherwise require the user's local proxy. Proxy selection must be protocol-specific (`HTTP_PROXY` for `http:`, `HTTPS_PROXY` for `https:`, `ALL_PROXY` as fallback) and must respect `NO_PROXY` for string, `URL`, and `Request` inputs. When using the proxy dispatcher, do not pass `AbortController.signal` directly into undici; keep caller-side timeout semantics in the wrapper to avoid premature `Request was cancelled` failures.
- Audiobook exports live under local export assets and are tracked by `audio_exports` plus `audio_export_segments`. Automatic chapter audio source selection uses `polishedText -> finalText -> draftText`. Exported audio files and preview files must not be written into formal story memory, chapter text, structured memory, or publish packages unless a future phase explicitly designs that flow.
- Audiobook export must stay cost-safe: do not allow a second pending/running export for the same project/chapter, enforce the active-export partial unique index with a friendly duplicate-export redirect, atomically lock failed/partial exports before retrying failed segments, stream TTS audio responses with a byte cap, reject non-audio 200 responses or mismatched audio signatures, and only allow unidentified `application/octet-stream` audio when the requested output format is raw `pcm`. Serve exported chapter audio only through the project-scoped `/projects/[projectId]/audio-assets` route after confirming the segment belongs to that project. The global `/audio-assets` route is for short TTS previews only, and preview cleanup should keep local assets bounded.
...
- The publish page should show only the latest publish package record by default. Older publish packages remain stored and are available from a collapsed history section; do not delete them just to simplify the UI.
...
- User-triggered AI generation should not block the UI while waiting for the model. Chapter beats, chapter drafts, chapter polish, summaries, pending update extraction, continuity checks, and publish package generation should start logged background tasks and return to a page that auto-refreshes. If a task needs follow-up writes, use a background completion callback after the task output is saved.
...
- Chapter polish prompt input has a single-call budget. Overlong source text is sent as a head/middle/tail excerpt with a visible task summary note; excerpt-only polish tasks are preview-only and must not be adoptable into `Chapter.polishedText`. Full long-chapter polish should become a separate segmented-polish feature rather than silently overloading the model context.
...
- AI cover image generation uses task type `cover_image_generation`. It starts a logged background image task from the publish page, uses the latest publish-package cover prompt or an author-entered prompt, and only writes to formal project cover fields after the author clicks "采用为封面". Adopted generated images are saved through the same local cover asset storage and then included in the Station Cat standard package.
...
- The chapter list should stay compact and metadata-only for long-form projects: do not load or render full chapter bodies on `/projects/[projectId]/chapters`; show chapter number, status, title, goal, word count, version count, and update time, with full text reserved for the chapter detail page.
