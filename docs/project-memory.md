...
- Current source app version is `0.1.25`; the latest formal personal-use macOS installer may lag source until the user approves review and requests packaging. Handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
