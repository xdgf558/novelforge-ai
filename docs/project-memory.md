...
- Phase 29: ending planning and closure readiness, including an outline-module “终局规划 / 收尾检查” panel, local ending-readiness signals from total word target, chapter progress, outline status, and unresolved foreshadows, plus logged `ending_planning_generation` AI draft tasks. AI may suggest remaining chapters, final story units, foreshadow recovery priorities, character endings, and conclusion tone, but it must not automatically modify formal outlines, foreshadows, timelines, or story memory. Each new plan records its generation chapter and a locally estimated validity window derived from remaining target words and observed chapter pace. After completion, only the latest non-empty plan in the `not_reviewed` or `adopted` state may be passed to later volume, story-unit, and chapter outline generation, and only when the target lies after the generation point and within that window. The reference is a non-overlapping, ending-preserving head/middle/tail excerpt capped at 6,000 characters and wrapped as untrusted model-output data. Authors may skip it for one outline generation or globally stop it by marking the latest plan ignored; an unusable latest plan never falls back to an older one. Formal outlines, settings, and finalized chapters always win conflicts. Prompt loading and retention share deterministic `createdAt` plus ID ordering, retention protects only the latest usable ending plan, and the outline page loads ending-plan history separately so the current reference remains reviewable and rejectable. Existing prompt template v1 rows upgrade to v2 on demand through `ensureDefaultPromptTemplate`; no schema migration is required.
