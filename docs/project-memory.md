...
- Current source app version is `0.1.115`; the latest personal-use macOS
  installer is
  `release/desktop/NovelForge-AI-0.1.115-mac-arm64.pkg`. This release has a
  ...
  explicitly authorize a new Apple upload for this rebuild, so `0.1.115` is
  ...
  previous `0.1.114` artifact was removed at the author's request. Future
...
- Next story-unit generation is a single-level operation. It must calculate the
  next formal unit number from existing units, keep the suggested start chapter
  fixed, continue the previous chapter inside the matching volume, and output
  only that one unit. Do not let a unit request regenerate the volume, repeat
  earlier units, invent nested sub-units, or append chapter outlines. Unit copy
  parsing must prefer the block whose range begins at the task's audited start
  chapter, and the handoff should preserve both `volumeNumber` and `unitNumber`
  when available.
