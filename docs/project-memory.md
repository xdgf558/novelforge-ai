...
- Phase 26: WeChat layout export enhancement on the project publish page. The deterministic formatter reads chapter text in `polishedText -> finalText -> draftText` order, supports “微信公众号正文粘贴版” and “微信公众号完整发布版”, and provides one-click copy plus TXT/Markdown/HTML downloads. Default behavior is “只排版，不改文”; AI publish packages may fill title/opening/ending candidate fields, but applying a candidate only changes the export form and never mutates formal chapter text or story memory.
