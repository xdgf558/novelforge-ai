...
- Current source app version is `0.1.50`; the latest formal personal-use macOS installer can lag behind source until the next packaging handoff. Handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
- Phase 27 added local backup creation under `/ai-settings`; backups are ZIP files containing the local SQLite database and generated assets, and deliberately exclude local `.env` API keys, TTS keys, image keys, proxy settings, and Station Cat tokens. Keep this invariant if backup/restore is extended later.
...
- TTS / audiobook export config is now editable at `/ai-settings`; the same local `.env` stores `TTS_PROVIDER_ID`, `TTS_API_KEY`, `TTS_API_BASE_URL`, `TTS_MODEL`, `TTS_VOICE_ID`, `TTS_VOICE_NAME`, `TTS_LANGUAGE_CODE`, `TTS_OUTPUT_FORMAT`, and `TTS_STYLE_PROMPT`. The active MVP provider is Google Gemini TTS with default base URL `https://generativelanguage.googleapis.com/v1beta`, default model `gemini-2.5-flash-preview-tts`, default voice `Kore`, language `cmn`, and WAV output. PPQ remains available only as legacy code for old records/tests and is no longer a user-facing audiobook provider.
...
- The publish page also has a deterministic WeChat layout export path for manual public-account publishing. It is separate from Station Cat upload and from AI publish packaging: layout export can use draft/polished/final chapter text, normalizes format for copy/download, and must not imply automatic WeChat publishing.
...
- The publish/export page should stay compact as publish assets grow: keep the book-cover tools, publish-package records, and project-export panels scannable; use short previews with explicit expand/collapse for long publish-package text instead of forcing full text into every card.
