...
- Current source app version is `0.1.21`; the latest formal personal-use macOS installer remains `0.1.20` until Phase 20 is reviewed and packaged. Handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
...
- AI chapter polish uses task type `chapter_polish_generation`. It reads the best available author text in this order: draft text, polished text, final text. Generated output is saved in `ai_tasks` and only moves into `Chapter.polishedText` after explicit author adoption. It must not directly overwrite `finalText`.
