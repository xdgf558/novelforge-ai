...
- Current source app version is `0.1.83`; the latest formal personal-use macOS installer is `release/desktop/NovelForge-AI-0.1.83-mac-arm64.pkg`. Future handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
...
- Current keychain has Developer ID Application signing available but no Developer ID Installer identity; until an Installer certificate is added, the PKG itself is unsigned. Copied or attribute-stripped Electron app bundles can fail verification after staging/PKG expansion, so prefer building the PKG directly from the signed `release/desktop/mac-arm64/NovelForge AI.app` payload unless a full signing/notarization pass is explicitly requested. Do not run a real overwrite install into `/Applications` for verification unless the user explicitly approves replacing the installed local app.
