...
- Current source app version is `0.1.90`; the latest formal personal-use macOS installer is `release/desktop/NovelForge-AI-0.1.90-mac-arm64.pkg`. Future handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
...
- Current keychain has Developer ID Application signing available but no Developer ID Installer identity, so the PKG itself remains unsigned. A Developer ID app can verify in the electron-builder output directory yet fail after an ordinary copy or PKG expansion; never accept the source-path verification alone. For a personal local build, copy the app into its final staging directory, re-sign every Electron component there (ad hoc is acceptable when keychain re-signing is unavailable) with hardened runtime and `build/entitlements.mac.plist`, then verify both a second copy and the expanded final PKG. Also run packaged desktop migrations against an isolated SQLite database. Do not run a real overwrite install into `/Applications` for verification unless the user explicitly approves replacing the installed local app.
