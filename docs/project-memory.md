...
- Chapter polish prompt input has a single-call budget. Overlong source text is sent as a head/middle/tail excerpt with a visible task summary note; full long-chapter polish should become a separate segmented-polish feature rather than silently overloading the model context.
