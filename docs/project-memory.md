...
- Current source app version is `0.1.98`; the latest formal personal-use macOS installer is `release/desktop/NovelForge-AI-0.1.98-mac-arm64.pkg`. Future handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
...
- Current keychain has both Developer ID Application and Developer ID Installer signing available. Personal local PKGs should sign the Electron app and the installer, then verify the installer certificate chain with `pkgutil --check-signature` and verify the app expanded from the final PKG with system-level `codesign --verify --deep --strict`; source-path verification alone is insufficient. Apple notarization uploads the private artifact externally, so do not notarize unless the user explicitly authorizes that upload. Until notarized, `spctl` will report `Unnotarized Developer ID` even though the Developer ID signatures and trusted timestamp are valid. Also run packaged desktop migrations against an isolated SQLite database. Do not run a real overwrite install into `/Applications` for verification unless the user explicitly approves replacing the installed local app.
