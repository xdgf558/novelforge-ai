...
- Current source app version is `0.1.35`; the latest formal personal-use macOS installer is still `0.1.34` until the user explicitly asks to package Phase 25. Handoff should leave only the final `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery folder unless the user explicitly asks for DMG/ZIP/update metadata.
...
- Audiobook export must stay cost-safe: do not allow a second pending/running export for the same project/chapter, enforce the active-export partial unique index, atomically lock failed/partial exports before retrying failed segments, stream TTS audio responses with a byte cap, reject non-audio 200 responses or mismatched audio signatures, and serve exported chapter audio only through the project-scoped `/projects/[projectId]/audio-assets` route after confirming the segment belongs to that project. The global `/audio-assets` route is for short TTS previews only, and preview cleanup should keep local assets bounded.
