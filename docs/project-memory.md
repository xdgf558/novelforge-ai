...
- Current source app version is `0.1.110`; the latest website-distribution
  macOS installer is
  `release/desktop/NovelForge-AI-0.1.110-mac-arm64.pkg`. This release has a
  hardened-runtime Developer ID Application signed payload and a Developer ID
  Installer signed PKG with trusted timestamps. The user explicitly authorized
  Apple notarization for website distribution, so both the application and
  final PKG are notarized and stapled; Gatekeeper accepts them as
  `Notarized Developer ID`. Future handoff should leave only the final
  `release/desktop/NovelForge-AI-<version>-mac-arm64.pkg` in the delivery
  folder unless the user explicitly asks for DMG/ZIP/update metadata.
