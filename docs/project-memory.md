...
- Phase 29: ending planning and closure readiness, including an outline-module “终局规划 / 收尾检查” panel, local ending-readiness signals from total word target, chapter progress, outline status, and unresolved foreshadows, plus logged `ending_planning_generation` AI draft tasks. AI may suggest remaining chapters, final story units, foreshadow recovery priorities, character endings, and conclusion tone, but it must not automatically modify formal outlines, foreshadows, timelines, or story memory. After a plan completes, the latest completed plan is automatically passed as bounded head/middle/tail reference context to later volume, story-unit, and chapter outline generation unless the author marks that latest plan ignored. Unreviewed and organized plans may guide later AI drafts without becoming formal memory; formal outlines, settings, and finalized chapters always win conflicts. Retention protects only the latest usable ending plan, while the outline page loads ending-plan history separately so the current reference remains reviewable and rejectable.
